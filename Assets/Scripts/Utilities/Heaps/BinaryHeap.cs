...
public class BinaryHeap<T>
{
    ...
    /// aka DecreaseKey
    /// </summary>
    //public void DecreasePriority(T n, double priority)
    //{
    ...
    /// <summary>
    /// Moves the node at the specified position upward, if it violates the Heap Property.
    ...
    public void TestHeapValidity()
    {
        for (int i = 1; i < count; i++)
            if (priorities[Parent(i)] > priorities[i])
                throw new InvalidOperationException("Heap violates the Heap Property at position " + i);
    }
    #endregion
}